function testjs
    ...
    browserDetail.loadString(getPlotlyHTML(detailCanvasID,dimA,dimA));
...
function html = getCanvasHTML( id, wid, hgt )
    ...
    html = [html,sprintf('<canvas id="%s" style = position : relative; style = display : block; width="%d" height="%d";></canvas>',id,wid,hgt)];
    html = [html,'<script type="text/javascript">'];
    html = [html,'window.addEventListener("resize", resizeCanvas, false);'];
    html = [html,sprintf('function resizeCanvas(e) {var canvas = document.getElementById("%s"); canvas.width = document.documentElement.clientWidth; canvas.height = document.documentElement.clientHeight; drawScreen();}</script>',id)];
    html = [html,'</body></html>'];
end

function html = getPlotlyHTML( id, wid, hgt )
    html = '<!DOCTYPE HTML><html>';
